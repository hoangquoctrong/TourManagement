    ...
    public static class TourInformationHandleModel
    {
        ...
        public static ObservableCollection<TourModel> GetTourListHaveInformation()
        {
            ...
            var tourlist = (from information in DataProvider.Ins.DB.TOUR_INFORMATION
                            join time in DataProvider.Ins.DB.TOUR_TIME on information.TOUR_INFORMATION_ID equals time.TOUR_INFORMATION_ID
                            where time.TOUR_TIME_DEPARTMENT_DATE > DateTime.Now && information.TOUR.TOUR_IS_EXIST.Equals("No")
                            select new
                            {
                                information.TOUR_ID
                            }).Distinct();
        ...
        {
            TOUR_INFORMATION information = DataProvider.Ins.DB.TOUR_INFORMATION.Where(x => x.TOUR_INFORMATION_ID == tourinformation_id).FirstOrDefault();

            return new TourModel()
            {
                TOUR_ID = information.TOUR_ID,
                TOUR_NAME = information.TOUR.TOUR_NAME,
                TOUR_CHARACTERISTIS = information.TOUR.TOUR_CHARACTERISTIS,
                TOUR_IS_EXIST = information.TOUR.TOUR_IS_EXIST,
                TOUR_STAR = (double)information.TOUR.TOUR_STAR,
                TOUR_TYPE = information.TOUR.TOUR_TYPE
            };
        }

        public static ObservableCollection<TourInformationModel> GetTourInformationListForTravelGroup(int tour_id)
        {
            ObservableCollection<TourInformationModel> InformationList = new ObservableCollection<TourInformationModel>();

            var informationList = from information in DataProvider.Ins.DB.TOUR_INFORMATION
                                  join time in DataProvider.Ins.DB.TOUR_TIME on information.TOUR_INFORMATION_ID equals time.TOUR_INFORMATION_ID
                                  where information.TOUR_ID == tour_id && time.TOUR_TIME_DEPARTMENT_DATE > DateTime.Now
                                  select information;
        ...
        public static TourInformationModel GetTourInformationTravelGroup(int tourinformation_id)
        {
            TOUR_INFORMATION item = DataProvider.Ins.DB.TOUR_INFORMATION.Where(x => x.TOUR_INFORMATION_ID == tourinformation_id).First();
